require_relative '../../../lib/puppet_x/transforms/xdt_transform_insert_after'
require 'nokogiri'
    ...
    describe '#transform' do
        ...
        context 'when more than 1 argument given' do
            it 'log warning and do no transforms' do
                ...
                XdtTransformInsertAfter.new.transform(source_node, transform_node, ['1','2'])
                expect(source_doc.root.elements.length).to eql(1)
            end
        end
        context 'when given xpath not found' do
            it 'log warning and do no transforms' do
                ...
                XdtTransformInsertAfter.new.transform(source_node, transform_node, ['/root/nope'])
                expect(source_doc.root.elements.length).to eql(1)
            end
        end
        context 'when given xpath is root' do
            it 'log warning and do no transforms' do
                ...
                XdtTransformInsertAfter.new.transform(source_node, transform_node, ['/root'])
                expect(source_doc.root.elements.length).to eql(1)
            end
        end
        context 'when given xpath found' do
            it 'transform node inserted after source node at parent' do
                ...
                XdtTransformInsertAfter.new.transform(source_node, transform_node, ['/root/a'])
